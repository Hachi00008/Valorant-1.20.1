...
# どちらのチームがアタッカーか確認+ラウンド入手
    ...
    execute if score $AttackerSide GameCore matches 0 run scoreboard players add @a[team=Red] money 300
    execute if score $AttackerSide GameCore matches 1 run scoreboard players add $Blue GetRound 1
    execute if score $AttackerSide GameCore matches 1 run scoreboard players add @a[team=Blue] GetRound 300
# 終了処理
